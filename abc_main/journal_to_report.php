	...
	while($company=mysql_fetch_array($C_name)){//每間公司
		...
		for($i=0;$i<$length;$i++){
			for($index=1;$index<=$table_rows[$i];$j++,$index++){
				if($i<5||$i>9){
					mysql_query("INSERT INTO `$table[$i]` VALUES($index,'$cid',$round_now,'$name[$j]',0)",$connect);
				}
				else{//財務狀況表先直接寫到下一期
					if($round_now>1){
						$result=mysql_query("SELECT price FROM `$table[$i]` WHERE `month`=$round_now-1 AND `cid`='$cid' AND `name`='$name[$j]'",$connect);
						$temp=mysql_fetch_array($result);
						mysql_query("INSERT INTO `$table[$i]` VALUES($index,'$cid',$round_now,'$name[$j]',$temp[0])",$connect);
					}
					else{ 
						mysql_query("INSERT INTO `$table[$i]` VALUES($index,'$cid',$round_now,'$name[$j]',0)",$connect);
					}
		...
		mysql_query("INSERT INTO `income_taxes` VALUES($index,'$cid',$round_now,0)",$connect);
		if($month+1>12){
			mysql_query("INSERT INTO `cash` VALUES($year+1,1,'$cid',0,0,$bankrupt[0])",$connect);
		}
		else{ 
			mysql_query("INSERT INTO `cash` VALUES($year,$month+1,'$cid',0,0,$bankrupt[0])",$connect);
		}
		$journal=mysql_query("SELECT * FROM journal ORDER BY `index`",$connect);
		while($decision_name=mysql_fetch_array($journal)){
			//mysql_select_db("report",$connect);如果報表放另一個資料庫的話可能會用到這行?
			switch ($decision_name['name']){
				case 'current_people':
					...
					if($sum!=0){
						...
						mysql_query("UPDATE `current_assets` SET `price`=$temp_result[0]-$sum WHERE `name`='111' AND `month`=$round_now AND `cid`='$cid'",$connect);
					
						$temp=mysql_query("SELECT price FROM operating_expenses WHERE `name`='512' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `operating_expenses` SET `price`=$temp_result[0]-$sum WHERE `name`='512' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					$netin-=$sum;
					break;
				case 'share':
					...
					if($sum!=0){//0代表無異動，不理它
						...
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `operating_expenses` SET `price`=$temp_result[0]-$sum WHERE `name`='525' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					$netin-=$sum;
					break;
				case 'share2':
					...
					if($sum!=0){//0代表無異動，不理它
						...
						$temp=mysql_query("SELECT price FROM operating_expenses WHERE `name`='524' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `operating_expenses` SET `price`=$temp_result[0]-$sum WHERE `name`='524' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					$netin-=$sum;
					break;
				case 'donate':
					...
					if($sum!=0){
						...
						$temp=mysql_query("SELECT price FROM other_expenses WHERE `name`='521' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `other_expenses` SET `price`=$temp_result[0]-$sum WHERE `name`='521' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					$netin-=$sum;
					break;
				case 'ad_a':
					...
					if($sum!=0){
						...
						$temp=mysql_query("SELECT price FROM operating_expenses WHERE `name`='515' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `operating_expenses` SET `price`=$temp_result[0]-$sum WHERE `name`='515' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					$netin-=$sum;
					break;
				case 'ad_b':
				...
				case 'purchase_materials':
					...
					if($sum!=0){
						...
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `current_assets` SET `price`=$temp_result[0]+$sum WHERE `name`='114' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp=mysql_query("SELECT price FROM current_liabilities WHERE `name`='213215' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `current_liabilities` SET `price`=$temp_result[0]+$sum WHERE `name`='213215' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					break;
				case 'purchase_machine':
					$for_cal=mysql_query("SELECT * FROM `machine` WHERE `buy_year`='$year' AND `buy_month`='$month' AND `cid`='$cid' ORDER BY `buy_year`,`buy_month`",$connect);
					...
					if($sum!=0){
						...
						$temp=mysql_query("SELECT price FROM `fixed_assets` WHERE `name`='141' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `fixed_assets` SET `price`=$temp_result[0]+$sum WHERE `name`='141' AND `month`=$round_now AND `cid`='$cid'",$connect);
						
						$temp=mysql_query("SELECT price FROM `current_assets` WHERE `name`='111' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `current_assets` SET `price`=$temp_result[0]-$sum WHERE `name`='111' AND `month`=$round_now AND `cid`='$cid'",$connect);
						
						mysql_query("UPDATE `investing_netin` SET `price`=0-$sum WHERE `name`='assets_purchase' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					break;
				case 'sell_machine'://之後要再乘上折舊
					...
					if($sum!=0){
						...
						$temp=mysql_query("SELECT price FROM `operating_expenses` WHERE `name`='518' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `operating_expenses` SET `price`=$temp_result[0]-$sell_lost WHERE `name`='518' AND `month`=$round_now AND `cid`='$cid'",$connect);
						
						$temp=mysql_query("SELECT price FROM current_assets WHERE `name`='111' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);//+$sell_lost 從下面式子移除2012/3/4 13:13
						mysql_query("UPDATE `current_assets` SET `price`=$temp_result[0]+$sell_cash WHERE `name`='111' AND `month`=$round_now AND `cid`='$cid'",$connect);
						
						mysql_query("UPDATE `investing_netin` SET `price`=$sell_cash WHERE `name`='assets_sale' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					$netin-=$sell_lost;
					break;
				case 'product_a':
...
*/							
				...
				if($rows_m!=0){
					...
					while($machine=mysql_fetch_array($sql_machine)){
						...
						if($depre!=0){
							$temp=mysql_query("SELECT price FROM `fixed_assets` WHERE `name`='142' AND `month`=$round_now AND `cid`='$cid'",$connect);
							$temp_result=mysql_fetch_array($temp);
							mysql_query("UPDATE `fixed_assets` SET `price`=$temp_result[0]-$depre WHERE `name`='142' AND `month`=$round_now AND `cid`='$cid'",$connect);
						//修改至帳目裡面
							
							$temp=mysql_query("SELECT price FROM operating_expenses WHERE `name`='525' AND `month`=$round_now AND `cid`='$cid'",$connect);
							$temp_result=mysql_fetch_array($temp);
							mysql_query("UPDATE `operating_expenses` SET `price`=$temp_result[0]-$depre WHERE `name`='525' AND `month`=$round_now AND `cid`='$cid'",$connect);
				...
				case 'long':
					...
					if($sum!=0){
						...
						$temp=mysql_query("SELECT price FROM `long-term_liabilities` WHERE `name`='212' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `long-term_liabilities` SET `price`=$temp_result[0]+$sum WHERE `name`='212' AND `month`=$round_now AND `cid`='$cid'",$connect);
					
						mysql_query("UPDATE `financing_netin` SET `price`=$sum WHERE `name`='long_pay' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					break;
				case 'short':
					...
					if($sum!=0){
						...
						$temp=mysql_query("SELECT price FROM `current_liabilities` WHERE `name`='211' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `current_liabilities` SET `price`=$temp_result[0]+$sum WHERE `name`='211' AND `month`=$round_now AND `cid`='$cid'",$connect);
					
						mysql_query("UPDATE `financing_netin` SET `price`=$sum WHERE `name`='short_pay' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					break;
				case 'repay':
					...
					if($sum!=0){
						...
						$temp=mysql_query("SELECT price FROM `financing_netin` WHERE `name`='long_pay' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `financing_netin` SET `price`=$temp_result[0]-$sum WHERE `name`='long_pay' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					break;
				case 'repay2':
					...
					if($sum!=0){
						...
						$temp=mysql_query("SELECT price FROM `financing_netin` WHERE `name`='short_pay' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `financing_netin` SET `price`=$temp_result[0]-$sum WHERE `name`='short_pay' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					break;
				case 'order'://銷貨收入AB要分開
					...
					if($sum!=0){
						$temp=mysql_query("SELECT price FROM current_assets WHERE `name`='113' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `current_assets` SET `price`=$temp_result[0]+$sum WHERE `name`='113' AND `month`=$round_now AND `cid`='$cid'",$connect);
					
						$temp=mysql_query("SELECT price FROM `operating_revenue` WHERE `name`='41' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `operating_revenue` SET `price`=$temp_result[0]+$pA_income WHERE `name`='41' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp=mysql_query("SELECT price FROM `operating_revenue` WHERE `name`='42' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `operating_revenue` SET `price`=$temp_result[0]+$pB_income WHERE `name`='42' AND `month`=$round_now AND `cid`='$cid'",$connect);
					
						$temp=mysql_query("SELECT price FROM `operating_costs` WHERE `name`='5111' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `operating_costs` SET `price`=0-$cal_array[0] WHERE `name`='5111' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp=mysql_query("SELECT price FROM `operating_costs` WHERE `name`='5112' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `operating_costs` SET `price`=0-$cal_array[1] WHERE `name`='5112' AND `month`=$round_now AND `cid`='$cid'",$connect);
						
						$temp=mysql_query("SELECT price FROM `current_assets` WHERE `name`='114' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `current_assets` SET `price`=$temp_result[0]-$cal_array[0]-$cal_array[1] WHERE `name`='114' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					$netin+=$sum;
                                        $netin-=$sum2;
					break;
				...
				case 'start_cash':
					...
					if($sum!=0){
						...
						$temp=mysql_query("SELECT price FROM `equity` WHERE `name`='31' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `equity` SET `price`=$temp_result[0]+$sum WHERE `name`='31' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					break;
				case 'break_contract':
				...
				case 'interest':
					...
					if($sum!=0){
						...
						$temp=mysql_query("SELECT price FROM `other_expenses` WHERE `name`='7' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `other_expenses` SET `price`=$temp_result[0]-$sum WHERE `name`='7' AND `month`=$round_now AND `cid`='$cid'",$connect);
					
						mysql_query("UPDATE `operating_netin` SET `price`=0-$sum WHERE `name`='interest' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					$netin-=$sum;
					break;
				case 'cash_increase':
					...
					if($sum!=0){
						...
						$temp=mysql_query("SELECT price FROM `equity` WHERE `name`='31' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `equity` SET `price`=$temp_result[0]+$stocks WHERE `name`='31' AND `month`=$round_now AND `cid`='$cid'",$connect);
						
						mysql_query("UPDATE `financing_netin` SET `price`=$sum WHERE `name`='cash_increase' AND `month`=$round_now AND `cid`='$cid'",$connect);
						
						//$temp=mysql_query("SELECT price FROM `equity` WHERE `name`='32' AND `month`=$round_now AND `cid`='$cid'",$connect);
						//$temp_result=mysql_fetch_array($temp);
						//$temp_capital=$temp_result[0];
						//if($temp_capital<$capital){
							//mysql_query("UPDATE `equity` SET `price`=$capital WHERE `name`='32' AND `month`=$round_now AND `cid`='$cid'",$connect);
						
							//$temp=mysql_query("SELECT price FROM `equity` WHERE `name`='35' AND `month`=$round_now AND `cid`='$cid'",$connect);
							//$temp_result=mysql_fetch_array($temp);
							//mysql_query("UPDATE `equity` SET `price`=$temp_result[0]+$capital+$temp_capital WHERE `name`='35' AND `month`=$round_now AND `cid`='$cid'",$connect);
						
										$temp=mysql_query("SELECT price FROM `equity` WHERE `name`='32' AND `month`=$round_now AND `cid`='$cid'",$connect);
										$temp_result=mysql_fetch_array($temp);
										mysql_query("UPDATE `equity` SET `price`=$temp_result[0]+$capital WHERE `name`='32' AND `month`=$round_now AND `cid`='$cid'",$connect);
						}
					break;
				case 'relationship_i':
					$dividend_cost=0;
					...
					break;
				case 'relationship_s':
                    $for_cal=mysql_query("SELECT * FROM `relationship_decision` WHERE`year`=$year AND `month`=$month AND `cid`='$cid' ORDERBY `year`, `month`",$connect);
                        if ($for_cal != NULL) {
                            while ($cal_array = mysql_fetch_array($for_cal)){
                                $r_s = explode("_", $cal_array['target']);
                                if ($r_s[0] == "supplier") {
                                    $result=mysql_query("SELECT * FROM `order_accept` WHERE`year`=$year AND `month`=$month AND`cid`='$cid'",$connect);
                                    $total_income=0;
                                    while($temp=mysql_fetch_array($result)){
                                        $order_no=$temp['order_no'];
					...
					if($sum!=0){
						...
						//mysql_query("UPDATE `financing_netin` SET `price`=$sum WHERE `name`='supplier_bonus' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					$netin-=$sum;
					break;
				case 'relationship_e':
					...
					if($sum!=0){
						...
						//mysql_query("UPDATE `financing_netin` SET `price`=0-$sum WHERE `name`='empolyee_bonus' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					$netin-=$sum;
					break;
				case 'dividend':
				...
				case 'treasury':
					...
					if($sum!=0){
						...
						$temp=mysql_query("SELECT price FROM equity WHERE `name`='33' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `equity` SET `price`=$temp_result[0]-$sum WHERE `name`='33' AND `month`=$round_now AND `cid`='$cid'",$connect);
						
						mysql_query("UPDATE `financing_netin` SET `price`=$sum*(-1) WHERE `name`='treasury' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					break;
				case 'AR':
					if($month-1==0)
						$for_cal=mysql_query("SELECT price,quantity FROM `order_accept` WHERE `year`=$year-1 AND `month`=12 AND `cid`='$cid' AND `accept`=1 ORDER BY `year`, `month`",$connect);
					else $for_cal=mysql_query("SELECT price,quantity FROM `order_accept` WHERE `year`=$year AND `month`=$month-1 AND `cid`='$cid' AND `accept`=1 ORDER BY `year`, `month`",$connect);
					while($cal_array = mysql_fetch_array($for_cal))
						if ($cal_array[0] != 0)
					...
					if($month-2==0)
						$for_cal=mysql_query("SELECT price,quantity FROM `order_accept` WHERE `year`=$year-1 AND `month`=12 AND `cid`='$cid' AND `accept`=1 ORDER BY `year`, `month`",$connect);
					else $for_cal=mysql_query("SELECT price,quantity FROM `order_accept` WHERE `year`=$year AND `month`=$month-2 AND `cid`='$cid' AND `accept`=1 ORDER BY `year`, `month`",$connect);
					while($cal_array = mysql_fetch_array($for_cal))
						if ($cal_array[0] != 0)
					...
					if($month-3==0)
						$for_cal=mysql_query("SELECT price,quantity FROM `order_accept` WHERE `year`=$year-1 AND `month`=12 AND `cid`='$cid' AND `accept`=1 ORDER BY `year`, `month`",$connect);
					else $for_cal=mysql_query("SELECT price,quantity FROM `order_accept` WHERE `year`=$year AND `month`=$month-3 AND `cid`='$cid' AND `accept`=1 ORDER BY `year`, `month`",$connect);
					while($cal_array = mysql_fetch_array($for_cal))
						if ($cal_array[0] != 0)
					...
					if($sum!=0){
						...
						mysql_query("UPDATE `operating_netin` SET `price`=$sum WHERE `name`='sales_income' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					break;
				case 'AP':
				...
				case 'tax':
					$tax=$netin*0.17;
					$sum=$tax;//上期資料之前已經寫入了
					$for_cal = mysql_query("SELECT `price` FROM `current_assets` WHERE `month`=$round_now-1 AND `cid`='$cid' AND `name`=115", $connect);
					$cal_array = mysql_fetch_array($for_cal);
					if($tax>$cal_array[0])
					...
					if($sum!=0){
						if($sum>0){
							$temp=mysql_query("SELECT price FROM income_taxes WHERE `month`=$round_now AND `cid`='$cid'",$connect);
							$temp_result=mysql_fetch_array($temp);
							mysql_query("UPDATE `income_taxes` SET `price`=$temp_result[0]-$sum WHERE `month`=$round_now AND `cid`='$cid'",$connect);
						
							$temp=mysql_query("SELECT price FROM current_assets WHERE `name`='115' AND `month`=$round_now AND `cid`='$cid'",$connect);
							$temp_result=mysql_fetch_array($temp);
							mysql_query("UPDATE `current_assets` SET `price`=$temp_result[0]-$tax_asset WHERE `name`='115' AND `month`=$round_now AND `cid`='$cid'",$connect);
							if($tax-$tax_asset>0){
								$temp=mysql_query("SELECT price FROM `long-term_liabilities` WHERE `name`='216' AND `month`=$round_now AND `cid`='$cid'",$connect);
								$temp_result=mysql_fetch_array($temp);
								mysql_query("UPDATE `long-term_liabilities` SET `price`=$temp_result[0]+$tax-$tax_asset WHERE `name`='216' AND `month`=$round_now AND `cid`='$cid'",$connect);
							}
						}
						else{
							$temp=mysql_query("SELECT price FROM income_taxes WHERE `month`=$round_now AND `cid`='$cid'",$connect);
							...
							$temp=mysql_query("SELECT price FROM current_assets WHERE `name`='115' AND `month`=$round_now AND `cid`='$cid'",$connect);
							$temp_result=mysql_fetch_array($temp);
							mysql_query("UPDATE `current_assets` SET `price`=$temp_result[0]-$sum WHERE `name`='115' AND `month`=$round_now AND `cid`='$cid'",$connect);
						...
						/*if($temp_result[0]+$sum>=0){
							...
							$temp=mysql_query("SELECT price FROM `long-term_liabilities` WHERE `name`='216' AND `month`=$round_now AND `cid`='$cid'",$connect);
							$temp_result=mysql_fetch_array($temp);
							mysql_query("UPDATE `long-term_liabilities` SET `price`=$temp_result[0]+$temp2 WHERE `name`='216' AND `month`=$round_now AND `cid`='$cid'",$connect);
						}*/
					}
					...
					if($month%12==1&&$tax_pay>0&&$year>1){//每年一月繳稅
						$temp=mysql_query("SELECT price FROM current_assets WHERE `name`='111' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `current_assets` SET `price`=$temp_result[0]-$tax_pay WHERE `name`='111' AND `month`=$round_now AND `cid`='$cid'",$connect);

						mysql_query("UPDATE `operating_netin` SET `price`=0-$tax_pay WHERE `name`='income_tax' AND `month`=$round_now AND `cid`='$cid'",$connect);
						/*這邊遞延所得稅資產應該都會跟應付所得稅扣抵了，所以扣應付所得稅就好了
						$temp=mysql_query("SELECT price FROM current_assets WHERE `name`='115' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `current_assets` SET `price`=0 WHERE `name`='115' AND `month`=$round_now AND `cid`='$cid'",$connect);
						*/
						$temp=mysql_query("SELECT price FROM `long-term_liabilities` WHERE `name`='216' AND `month`=$round_now AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `long-term_liabilities` SET `price`=$temp_result[0]-$tax_pay WHERE `name`='216' AND `month`=$round_now AND `cid`='$cid'",$connect);
					}
					break;
				case 'netin':
					$sum=$netin-$tax;
					$temp=mysql_query("SELECT price FROM current_assets WHERE `name`='111' AND `month`=$round_now AND `cid`='$cid'",$connect);
					$temp_result=mysql_fetch_array($temp);
					mysql_query("UPDATE `current_assets` SET `price`=$temp_result[0] WHERE `name`='111' AND `month`=$round_now AND `cid`='$cid'",$connect);
					...
					if($month+1>12){
						mysql_query("UPDATE `cash` SET `cash`=$cash WHERE `year`=$year+1 AND `month`=1 AND `cid`='$cid'",$connect);
						mysql_query("UPDATE `cash` SET `netin`=$netin WHERE `year`=$year+1 AND `month`=1 AND `cid`='$cid'",$connect);
					}
					else{
						mysql_query("UPDATE `cash` SET `cash`=$cash WHERE `year`=$year AND `month`=$month+1 AND `cid`='$cid'",$connect);
						mysql_query("UPDATE `cash` SET `netin`=$netin WHERE `year`=$year AND `month`=$month+1 AND `cid`='$cid'",$connect);
					}
					if($round_now==1)
						mysql_query("UPDATE `cash_balance_last` SET `price`=20000000 WHERE `month`=$round_now AND `cid`='$cid'",$connect);
					else{
						$temp=mysql_query("SELECT price FROM current_assets WHERE `name`='111' AND `month`=$round_now-1 AND `cid`='$cid'",$connect);
						$temp_result=mysql_fetch_array($temp);
						mysql_query("UPDATE `cash_balance_last` SET `price`=$temp_result[0] WHERE `month`=$round_now AND `cid`='$cid'",$connect);
					}
					break;
			}//end of switch
			$sum=0;//千萬記得要歸零...
		...
		//營業費用付現
						$for_cal = mysql_query("SELECT `price` FROM `operating_expenses` WHERE `name`='518' AND `cid` = '$cid' AND `month` = $round_now", $connect);
						$cal_array=mysql_fetch_array($for_cal);
		...
		$cal_array=mysql_fetch_array($for_cal);
		mysql_query("UPDATE `operating_netin` SET `price`=$cal_array[0] WHERE `name`='other_exp' AND `month`=$round_now AND `cid`='$cid'",$connect);
		
		//判斷是否現金增資
		if($month+1>12){
			$temp=mysql_query("SELECT cash FROM `cash` WHERE `year`=$year+1 AND `month`=1 AND `cid`='$cid'",$connect);
		}
		else{ 
			$temp=mysql_query("SELECT cash FROM `cash` WHERE `year`=$year AND `month`=$month+1 AND `cid`='$cid'",$connect);
		}
		$temp_result=mysql_fetch_array($temp);		
		$round = ($year-1)*12 + $month;
			//在外流通股數

		//$cash = mysql_query("SELECT `cash_increase` FROM `fund_raising` WHERE `year`=$year AND `month`=$month-1 AND `cid`='$cid'",$connect);
		//$outside_stock = mysql_fetch_array($cash);	
		//$stock_carter = $outside_stock[0]/10;
		//mysql_query("INSERT INTO `outside_stock` VALUES($year, $month, '$cid', '$stock_carter')",$connect);
		//mysql_query("UPDATE `outside_stock` outside_stock`=  $outside_stock[0]  WHERE `cid`='$cid' AND `year`=$year AND `month`=$month",$connect);		
		
		
		if($temp_result[0]<0){
			$outstock=(20000000-$temp_result[0])/10;
       		mysql_query("UPDATE `fund_raising` SET `cash_increase`=20000000-$temp_result[0] WHERE `year`=$year AND `month`=$month AND `cid`='$cid'",$connect);
			if($month+1>12){
				$temp=mysql_query("UPDATE `cash` SET `cash`=20000000 WHERE `year`=$year+1 AND `month`=1 AND `cid`='$cid'",$connect);
			}
			else{ 
				$temp=mysql_query("UPDATE `cash` SET `cash`=20000000 WHERE `year`=$year AND `month`=$month+1 AND `cid`='$cid'",$connect);
			}
			//再跑一次現金增資對報表的影響
			$for_cal=mysql_query("SELECT `cash_increase` FROM `fund_raising` WHERE `year`=$year AND `month`=$month AND `cid`='$cid' ORDER BY `year`, `month`",$connect);
			$cal_array=mysql_fetch_array($for_cal);
