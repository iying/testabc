...
function report($month) {
    $args = array("current_people" => "招解聘", "share" => "企業共享價值", "share2" => "企業共享價值", "donate" => "捐款",
        ...
        "relationship_e" => "員工關係管理", "relationship_i" => "投資人關係管理", "dividend" => "現金股利", "depreciation" => "機具折舊", "AR" => "應收帳款", "AP" => "應付帳款", "treasury" => "買回庫藏股"); //決策項目對照
    ...
    $subject = array("current_people" => "薪資費用", "share" => "管理及總務費用", "share2" => "研究發展費用", "donate" => "捐款",
        ...
        "relationship_e" => "員工福利費用", "relationship_i" => "管理及總務費用", "dividend" => "保留盈餘", "depreciation" => "製造費用-折舊", "AR" => "現金", "AP" => "應付帳款", "treasury" => "庫藏股"); //會計科目對照(借)
...
//以下為固定欄位
    ...
    echo "<th width = 90 colspan='2'>會計科目</th><th width = 80>借方</th><th width = 80>貸方</th><th colspan='2' style='text-align:center'>決策</th></tr></thead>";
                ...
                if ($for_cal != NULL) {
                    while ($cal_array = mysql_fetch_array($for_cal)){
                        ...
                        if ($r_s[0] == "supplier") {
                            ...
                            else $result = mysql_query("SELECT * FROM `order_accept` WHERE`year`=$year AND `month`=$month AND `cid`='$cid'",$connect);
                            $total_income = 0;
                            while ($temp = mysql_fetch_array($result)) {
                                $order_no = $temp['order_no'];
        ...
        if ($sum != 0) {//0代表無異動，不顯示 && $decision_name['name'] != "product_a"
            ...
            elseif ($decision_name['name'] == "sell_machine") {
                if ($catalog_temp != $catalog[$decision_name['name']]){//同種決策類別只顯示一次
                    $flag++;
					flag($flag);
					echo "<td></td><td style='text-align:left;border-right:0px;'>" . $subject[$decision_name['name']] . "</td><td style='text-align:right'>" . number_format($sell_cash) . "</td><td></td><td>" . $catalog[$decision_name['name']] . "</td><td>" . $args[$decision_name['name']] . "</td></tr>";
				}else{ 
					$flag++;
					flag($flag);
					echo "<td></td><td style='text-align:left;border-right:0px;'>" . $subject[$decision_name['name']] . "</td><td style='text-align:right'>" . number_format($sell_cash) . "</td><td></td><td></td><td>" . $args[$decision_name['name']] . "</td></tr>";
				}
				$flag++;
				flag($flag);
                echo "<td></td><td style='text-align:left;border-right:0px;'>累積折舊</td><td style='text-align:right'>" . number_format($depreciation) . "</td><td></td><td></td><td></td></tr>";
                $flag++;
				flag($flag);
				echo "<td></td><td style='text-align:left;border-right:0px;'>處分損失</td><td style='text-align:right'>" . number_format($sell_lost) . "</td><td></td><td></td><td></td></tr>";
                $flag++;
				flag($flag);
				echo "<td></td><td style='text-align:left;border-right:0px;'>" . $subject2[$decision_name['name']] . "</td><td></td><td style='text-align:right'>" . number_format($machine) . "</td><td></td><td></td></tr>";
                $catalog_temp = $catalog[$decision_name['name']];
            }
			//ok
            elseif ($decision_name['name'] == "product_plan") {
				...
				$flag++;
				flag($flag);
                echo "<td></td><td style='text-align:left;border-right:0px;'>" . $subject2[$decision_name['name']] . "</td><td></td><td style='text-align:right'>" . number_format($sum) . "</td><td></td><td></td></tr>";
                $flag++;
				flag($flag);
                ...
                $flag++;
				flag($flag);
				echo "<td></td><td style='text-align:left;border-right:0px;'>薪資費用</td><td></td><td style='text-align:right'>" . number_format($direct_labor) . "</td><td></td><td></td></tr>";
				$flag++;
				flag($flag);
				...
				$flag++;
				flag($flag);
				echo "<td></td><td style='text-align:left;border-right:0px;'>累計折舊</td><td></td<td></td>><td style='text-align:right'>" . number_format($depreciation) . "</td><td></td><td></td></tr>";
				$flag++;
				flag($flag);
            ...
            elseif ($decision_name['name'] == "cash_increase") {
                ...
                if ($capital == 0){
					...
					$flag++;
					flag($flag);
                    echo "<td></td><td style='border-right:0px;'></td><td style='text-align:left;border-left:0px;'>資本公積</td><td></td><td style='text-align:right'>" . number_format($capital) . "</td><td></td><td></td></tr>";
                } elseif ($capital < 0) {
					$flag++;
					flag($flag);
                    echo "<td></td><td style='border-right:0px;'></td><td style='text-align:left;border-left:0px;'>資本公積</td><td style='text-align:right'>" . number_format($capital*(-1)) . "</td><td></td><td></td><td></td></tr>";
                    $flag++;
					flag($flag);
					echo "<td></td><td style='text-align:left;border-right:0px;'>" . $subject2[$decision_name['name']] . "</td><td style='border-left:0px;'></td><td style='text-align:right'>" . number_format($stocks) . "</td><td></td><td></td></tr>";
                }$catalog_temp = $catalog[$decision_name['name']];
            }
				...
				if($tax>0){
					...
					$flag++;
					flag($flag);
					echo "<td></td><td style='border-right:0px;'></td><td style='text-align:left;border-left:0px;'>遞延所得稅資產</td><td></td><td style='text-align:right'>" . number_format($tax_asset) . "</td><td></td><td></td></tr>";
					if($tax-$tax_asset>0){ 
						$flag++;
						flag($flag);
